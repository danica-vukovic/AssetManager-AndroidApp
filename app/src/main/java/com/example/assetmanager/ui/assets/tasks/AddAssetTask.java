...
public class AddAssetTask extends AsyncTask<Void, Void, Boolean> {
    private final WeakReference<AddAssetFragment> fragmentWeakReference;
    private final Asset asset;

    public AddAssetTask(AddAssetFragment fragment, Asset asset) {
    ...
    protected Boolean doInBackground(Void... voids) {
        ...
        if (fragment != null && asset != null) {
            try {
                ...
                long res = db.getAssetDao().insertAsset(asset);
                result = res > 0;
            } catch (Exception e) {
                Log.e("AddAssetTask", "Error adding asset", e);
            }
        }
        return result;
    }
    ...
    protected void onPostExecute(Boolean success) {
        ...
        if (fragment != null) {
            if (success) {
                Toast.makeText(fragment.getContext(), R.string.asset_added_successfully, Toast.LENGTH_SHORT).show();
                fragment.clearFields();
            } else {
                Toast.makeText(fragment.getContext(), R.string.failed_to_add_asset, Toast.LENGTH_SHORT).show();
            }
        }
    }
}
